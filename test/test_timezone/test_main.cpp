...
static void test_lookup_borders(void) {
  ...
  chk(-54.42f, 3.36f, 0, 0, false, false);         // Bouvetøya  UTC+0 (no IANA zone)
  TzResult bv;
  tz_lookup(-54.42f, 3.36f, bv);
  // Hand-drawn polygon (editor) wins over the box; both carry GMT0.
  TEST_ASSERT_EQUAL_STRING("Atlantic/Bouvet", bv.name);
}
